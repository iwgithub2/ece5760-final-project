...
module log_add_rom (
    input clk,
    input [9:0] address,      // 10-bit address for 1024 entries
    output reg [31:0] data_out // 32-bit data (fixed point or float representation)
);

    // 1. Declare the memory array (Width x Depth)
    reg [31:0] rom_memory [0:1023];

    // 2. Tell Quartus to load the hex file during synthesis
    initial begin
        $readmemh("log_lut.hex", rom_memory);
    end

    // 3. Synchronous Read (Required for M10K Block RAM inference)
